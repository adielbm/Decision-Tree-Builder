...
function App() {
  ...
  const convertTreeToMermaid = (node, nodeId = 'root', parentId = null, isOptionNode = false) => {
    if (!node) return '';
    
    let mermaidCode = '';
    let connections = '';
    let nodeClasses = new Set();
    let classAssignments = '';
    
    // Escape and truncate text for Mermaid
    ...
    if (node.options) {
      // This is a decision node - only use question if it exists and is not empty
      const questionText = node.question_for_options?.trim();
      if (!questionText) {
        // Skip this node if no question - connect parent directly to children
        node.options.forEach((option, index) => {
          if (option.options) {
            // Option leads to another question - create option node first
            const optionNodeId = `${nodeId}_opt_${index}`;
            const optionLabel = escapeText(option.title);
            mermaidCode += `    ${optionNodeId}["${optionLabel}"]\n`;
            nodeClasses.add('option');
            classAssignments += `    class ${optionNodeId} option\n`;
            
            if (parentId) {
              connections += `    ${parentId} --> ${optionNodeId}\n`;
            }
            
            // Then create the next question node
            const childId = `${nodeId}_${index}`;
            const childResult = convertTreeToMermaid(option, childId, optionNodeId, true);
            mermaidCode += childResult.nodes;
            connections += childResult.connections;
            classAssignments += childResult.classAssignments;
            childResult.nodeClasses.forEach(cls => nodeClasses.add(cls));
          } else {
            // Option leads to terminal - create option node first
            const optionNodeId = `${nodeId}_opt_${index}`;
            const optionLabel = escapeText(option.title);
            mermaidCode += `    ${optionNodeId}["${optionLabel}"]\n`;
            nodeClasses.add('option');
            ...
            // Then create terminal node
            const childId = `${nodeId}_${index}`;
            const childResult = convertTreeToMermaid(option, childId, optionNodeId, false);
            mermaidCode += childResult.nodes;
            connections += childResult.connections;
            classAssignments += childResult.classAssignments;
            childResult.nodeClasses.forEach(cls => nodeClasses.add(cls));
        ...
        // Questions use rhombus shape
        const nodeLabel = escapeText(questionText);
        mermaidCode += `    ${nodeId}{"${nodeLabel}"}\n`;
        nodeClasses.add('question');
        classAssignments += `    class ${nodeId} question\n`;
      ...
      // This is a terminal node - use title as the node label
      const nodeLabel = escapeText(node.title);
      mermaidCode += `    ${nodeId}(("${nodeLabel}"))\n`;
      nodeClasses.add('terminal');
      classAssignments += `    class ${nodeId} terminal\n`;
  ...
  const generateMermaidDiagram = () => {
    ...
    const result = convertTreeToMermaid(tree);
    
    // DRY styling - define classes only once
    let styling = '';
    if (result.nodeClasses.has('question')) {
      styling += `    classDef question fill:#e1bee7,stroke:#7b1fa2,stroke-width:2px,color:#000\n`;
    }
    if (result.nodeClasses.has('option')) {
      styling += `    classDef option fill:#ffe0b2,stroke:#f57c00,stroke-width:2px,color:#000\n`;
    }
    if (result.nodeClasses.has('terminal')) {
      styling += `    classDef terminal fill:#bbdefb,stroke:#1976d2,stroke-width:2px,color:#000\n`;
    }
    
    return `flowchart TD\n${result.nodes}${result.connections}\n${styling}${result.classAssignments}`;
  };
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'mermaid-code':
        const mermaidCode = tree ? generateMermaidDiagram() : "No tree loaded.";
